...
class CSVTemplateGenerator:
    ...
    def _initialize_templates(self) -> Dict[TemplateType, List[TemplateField]]:
        ...
        return {
            ...
            TemplateType.PORTUGUESE: [
                TemplateField("contratoId", "ID_Contrato", "Identificador do contrato", "123456"),
                TemplateField("dataInicio", "Data_de_Inicio", "Data de início do período", "01/07/2024"),
                TemplateField("dataFim", "Data_de_Fim", "Data de fim do período", "31/07/2024"),
                TemplateField("dataPagamento", "Data_Pagamento", "Data do pagamento", "28/07/2024"),
                TemplateField("tipoRecibo", "Tipo_de_Recibo", "Tipo de recibo", "renda"),
                TemplateField("valor", "Valor_Euros", "Valor em euros", "850,00"),
                TemplateField("descricao", "Descricao", "Descrição do pagamento", "Renda mensal", False),
            ],
            ...
            TemplateType.INHERITANCE: [
                ...
                TemplateField("receiptType", "Tipo_Recibo", "Type of receipt", "rent"),
                TemplateField("value", "Valor", "Receipt amount", "850.00"),
                TemplateField("inheritancePercentage", "Percentagem_Heranca", "Inheritance percentage", "50.00", False),
                TemplateField("coOwnerNIF", "NIF_Coproprietario", "Co-owner NIF", "987654321", False),
            ],
            ...
            TemplateType.MULTI_TENANT: [
                ...
                TemplateField("toDate", "Data_Fim", "Receipt period end date", "2024-07-31"),
                TemplateField("receiptType", "Tipo_Recibo", "Type of receipt", "rent"),
                TemplateField("totalValue", "Valor_Total", "Total receipt amount", "1200.00"),
                TemplateField("tenant1Value", "Valor_Inquilino1", "Tenant 1 amount", "600.00", False),
                TemplateField("tenant2Value", "Valor_Inquilino2", "Tenant 2 amount", "600.00", False),
                TemplateField("tenant1NIF", "NIF_Inquilino1", "Tenant 1 NIF", "123456789", False),
                TemplateField("tenant2NIF", "NIF_Inquilino2", "Tenant 2 NIF", "987654321", False),
            ],
            ...
            TemplateType.BUSINESS: [
                ...
                TemplateField("toDate", "Data_Fim", "Receipt period end date", "2024-07-31"),
                TemplateField("receiptType", "Tipo_Recibo", "Type of receipt", "rent"),
                TemplateField("value", "Valor", "Receipt amount", "1500.00"),
                TemplateField("businessName", "Nome_Empresa", "Business name", "Tech Solutions Lda", False),
                TemplateField("businessNIF", "NIF_Empresa", "Business NIF", "507123456", False),
                TemplateField("vatRate", "Taxa_IVA", "VAT rate", "23.00", False),
                TemplateField("netValue", "Valor_Liquido", "Net value", "1219.51", False),
                TemplateField("vatValue", "Valor_IVA", "VAT value", "280.49", False),
            ],
        }
    ...
    def _generate_sample_data(self, template_type: TemplateType, count: int) -> List[Dict[str, str]]:
        ...
        if template_type == TemplateType.PORTUGUESE:
            base_samples = self.sample_data.get("portuguese", [])
        elif template_type == TemplateType.BUSINESS:
            base_samples = self.sample_data.get("business", [])
        else:
        ...
        for i in range(count):
            if i < len(base_samples):
                ...
                if base_samples:
                    ...
                    for key in ['contractId', 'contratoId']:
                        if key in new_sample:
                            original_id = new_sample[key]
                            if original_id.isdigit():
                                # Numeric ID - add increment
                                base_id = int(original_id)
                                new_sample[key] = str(base_id + i)
                            else:
                                # Non-numeric ID - append increment
                                new_sample[key] = f"{original_id}_{i+1:02d}"
                            break
    ...
    def generate_multiple_templates(self, output_dir: str) -> Tuple[bool, str, Dict[str, bool]]:
        ...
        try:
            ...
            templates_to_generate = [
                ...
                (TemplateType.PORTUGUESE, "template_portugues.csv", "Portuguese CSV template"),
                (TemplateType.INHERITANCE, "inheritance_template.csv", "Inheritance template"),
                (TemplateType.MULTI_TENANT, "multi_tenant_template.csv", "Multi-tenant template"),
                (TemplateType.BUSINESS, "business_template.csv", "Business template"),
            ]
            
            # Add sample data template as a special case of detailed template with samples
            templates_to_generate.append((TemplateType.DETAILED, "sample_data.csv", "Sample data template"))
            ...
            for template_type, filename, description in templates_to_generate:
                ...
                if filename == "sample_data.csv":
                    ...
                    config = TemplateConfig(
                        ...
                        include_samples=(template_type == TemplateType.PORTUGUESE),
                        include_portuguese=(template_type == TemplateType.PORTUGUESE),
                        include_descriptions=True,
                        sample_count=3 if template_type == TemplateType.PORTUGUESE else 2
                    )
